    ...
    class GamesController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function server(\Illuminate\Http\Request $request, $game)
        {
            ...
            if( settings('check_active_tab') ) 
            {
                ...
                {
                    echo '{"responseEvent":"error","responseType":"start","serverResponse":"Wrong sessionId", "desc:":"Missing sessionID"}';
                    exit();
                }
                if( $subssession && !$subssession->active ) 
                {
                    echo '{"responseEvent":"error","responseType":"error","serverResponse":"Wrong sessionId", "desc:":"Inactive session"}';
                    exit();
                }
            }
            if( !$subssession ) 
