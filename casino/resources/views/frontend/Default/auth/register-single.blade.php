...
@section('content')
  <div id="login-box">
    ...
    <div id="login-inner-box">
      ...
      <form name="form" method="post" class="form-validation" action="<?= route('frontend.register.post') ?>">
        <input type="hidden" value="<?= csrf_token() ?>" name="_token">
        <div class="text-danger wrapper text-center" ng-show="authError"></div>
        <div class="input-group mb-3">
          <label class="input-group-text" for="username">@lang('app.username')</label>
          <input type="text" class="form-control" name="username" id="username" placeholder="@lang('app.email_or_username')">
        </div>
      @if(settings('use_email'))
        <div class="input-group mb-3">
          <label class="input-group-text" for="email">@lang('app.email')</label>
          <input type="email" class="form-control" name="email" id="email" placeholder="@lang('app.email_or_email')">
        </div>
      @endif
        <div class="input-group mb-3">
        ...
        <div class="input-group mb-3">
          <label class="input-group-text" for="password_confirmation">@lang('app.confirm_password')</label>
          <input type="password" class="form-control" name="password_confirmation" id="password_confirmation" placeholder="@lang('app.confirm_password')">
        </div>
        <div class="d-grid gap-2 mb-3">
          <button type="submit" class="btn btn-login btn-lg btn-primary btn-block">@lang('app.register')</button>
        </div>
        <div class="text-center">
          <span style="margin-top: 5px;"><a href="{{url('password/remind')}}" >Forgot password</a> | <a href="{{url('login')}}" >Back to Login</a></span>
        </div>
      </form>
    </div>
